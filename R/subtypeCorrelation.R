  ...
  function (eset, sig, method=c("pearson", "spearman", "kendall"), weighted=FALSE, condensed=TRUE, plot=TRUE, resdir="cache", nthread=1, sig.method, sig.scaling, label=c("symbol", "entrez")) {
    ...
    if (plot) {
      ...
      if (condensed) {
        ...
        newxx <- hclust(dist(rr[[1]]))
        for (i in 1:length(rr)) {
          xx <- rr[[i]]
#           newxx <- hclust(dist(xx))
          xx <- xx[newxx$order,newxx$order]
          ...
          if( i >1){
            ...
            print(plotpic, split = c((i-((ceiling(i/3)-1)*3)),ceiling(i/3), 3, 2), newpage=FALSE)
            
          } else {
            
            print(plotpic, split = c((i-((ceiling(i/3)-1)*3)),ceiling(i/3), 3, 2))
          }

        }
        dev.copy2pdf(width=nc * 5, height=nr * 5,device = dev1, file = file.path(resdir,"subtype_correlation.pdf"))
        dev.off()
      } else {
        for (i in 1:length(rr)) {
