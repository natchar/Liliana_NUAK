...
`subtypeSurvival` <- 
  function (eset, sig, plot=FALSE, weighted=FALSE, surv.type=c("dfs", "rfs", "dmfs", "tdm"), time.cens, condensed=TRUE, resdir="cache", nthread=1, sig.method, sig.scaling, label=c("symbol", "entrez")) {
    ...
    rownames(expr)[grep("geneid",names(sig))]<-fData(eset)[names(sig)[grep("geneid",names(sig))], "SYMBOL"]
    } else{
      rownames(expr) <- names(sig)
    }  
    ...
    if(label == "symbol"){
      names(rr) <- names(sig)
#       lapply(grep("geneid.", names(sig)), function(i) {
#         names(rr)[i] <- fData(eset)[names(sig)[i], "SYMBOL"]
#       })
       names(rr)[grep("geneid",names(sig))]<-fData(eset)[names(sig)[grep("geneid",names(sig))], "SYMBOL"]
    } else{
      names(rr) <- names(sig)
    }  
